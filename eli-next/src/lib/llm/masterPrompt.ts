...
/**
 * Master Prompt Émergent v3 — profil hydraté + adaptation MATIÈRE/NIVEAU + VOIX EN PREMIER.
 * @param focusSubject matière choisie par l'élève dans l'interface (peut être null).
 */
export function buildSystemPrompt(profile: Profile, progress: Progress[], focusSubject?: string | null): string {
  const ctx = {
  ...
  return [
    ...
    'priorité coefficient×zone rouge avant examen, micro-bilan + UNE prochaine étape en fin de session, encouragement sans complaisance.',
    ...
    'DEUX MODES, tu choisis selon la demande : (1) par défaut, méthode SOCRATIQUE — tu guides par questions sans donner la réponse d\'emblée ;',
    ...
    'FIN DE SESSION : émets un bloc JSON {chapitre_travaille, reussites, erreurs_types, statut_propose, prochaine_etape}.',
  ].filter(Boolean).join('\n');
}
